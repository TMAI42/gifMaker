...
int GifMaker::InitFrames(int width, int height, AVPixelFormat inputPixelFormat) {
	...
	if ((size = av_image_alloc(pictureInput->data, pictureInput->linesize,
		...
		printf("allocated picture of size %d (ptr %x), linesize %d %d %d %d\n", size, pictureInput->data[0], pictureInput->linesize[0], pictureInput->linesize[1], pictureInput->linesize[2], pictureInput->linesize[3]);
	...
	pictureRGB24.swap(pictureTmp2);
	pictureRGB24->format = AV_PIX_FMT_RGB24;

	if ((av_image_alloc(pictureRGB24->data, pictureRGB24->linesize,
		outCodecContext->width, outCodecContext->height, (AVPixelFormat)pictureInput->format, 32)) < 0) {

		throw std::exception("can not allocate temp frame");
	}
	else
		printf("allocated picture of size %d (ptr %x), linesize %d %d %d %d\n", 0, pictureRGB24->data[0], pictureRGB24->linesize[0], pictureRGB24->linesize[1], pictureRGB24->linesize[2], pictureRGB24->linesize[3]);

	pictureRGB24->height = height;
	pictureRGB24->width = width;
...
void GifMaker::InitPixelConvertionContext() {
	...
	swsContextToRGB24 = sws_getContext(outCodecContext->width, outCodecContext->height, (AVPixelFormat)pictureInput->format,
		outCodecContext->width, outCodecContext->height, AV_PIX_FMT_YUV420P,
		SWS_BILINEAR, NULL, NULL, NULL);

	/* get sws context for RGB24->RGB8 conversion */
	...
	if (!swsContextToRGB24 || !swsContextToRGB8) {
...
void GifMaker::AddFrame(std::shared_ptr<AVFrame> frame) {
	/* copy the buffer */
	//memcpy(pictureInput->data[0], frame->data[0], size);
	...
	sws_scale(swsContextToRGB24,
		pictureInput->data,
		pictureInput->linesize,
		0, outCodecContext->height,
		pictureRGB24->data,
		pictureRGB24->linesize);
